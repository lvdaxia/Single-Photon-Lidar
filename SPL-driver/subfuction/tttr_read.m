% 20240722
% 可用于T2和T3数据的实时读取, 支持HydraHarp/TimeHarp260两种设备
% by 吕林杰
% 全局变量包括：data_mat，cnt_ph，cnt_ov，RecNum，sync_time
%% Decoder functions
function [ph_record_time,ph_flight_time]=tttr_read(buffer, Mode)
...
function ReadHT2(buffer,isT2)
    Version=2;%%%%%% HdraHarp 400和timeharp 260的版本均为2
    global RecNum;
    global OverflowCorrection;
    ...
    for i=1:length(buffer)
        ...
        %   +-------------------------------+  +-------------------------------+
        dtime = double(bitand(T2Record,33554431));   % the last 25 bits:
        %   +-------------------------------+  +-------------------------------+
        ...
        timetag = double(OverflowCorrection) + dtime;
...
function ReadHT3(buffer,isT2)
    ...
    for i = 1:length(buffer)
        ...
        %   |x|x|x|x|x|x|x|x|x|x|x|x|x|x|x|x|  |x|x|x|x|x|x|x|x|x|x|x|x|x|x|x|x|
        %   +-------------------------------+  +-------------------------------+
        nsync = double(bitand(T3Record,1023));       % the lowest 10 bits:
        %   +-------------------------------+  +-------------------------------+
        %   | | | | | | | | | | | | | | | | |  | | | | | | |x|x|x|x|x|x|x|x|x|x|
        %   +-------------------------------+  +-------------------------------+
...
function GotPhoton(TimeTag, Channel, DTime,isT2)
  ...
  if(isT2)
      ...
      if Dtime_by_T2
%           data_mat(cnt_ph,:)=[TimeTag Dtime_by_T2];
