...
f=0.001;
for i=1:21
    path_name='C:\Users\honor\Desktop\20240906\holdoff_1us_freerunning\';
    file_name=['20240909-',num2str((21+i)*100)];
    
    name1=[file_name,'.ptu'];
    multi_PTU2outfile(path_name, name1);
    
    name2=[path_name,file_name,'.out'];
    [data_mat]=single_T2_outfile_2_mat(name2,f);
    
    save([path_name,file_name, '.mat'],'data_mat');
    disp('i=');
    disp(i);
%     waitbar(i/length(i));
end
...
function [data_mat]=single_T2_outfile_2_mat(filename,f)
    matT2 = single_T2_2_mat(filename);
    data_mat=T2mat_2_T3(matT2,f);
end
...
function data_mat=T2mat_2_T3(file,f)
%     f=0.025;
    s=file(:,2);
    CH=file(:,1);
    [length,~]=size(file) ;
